...
export default function SideNavigation({
  ...
  return (
    <div className="hidden h-screen w-[16.5rem] flex-shrink-0 flex-col items-start justify-between bg-white transition-all duration-200 dark:bg-kanbanDarkGrey md:flex">
      <div>
        <svg
        ...
        <ul>
          {boards.map((board, idx) => (
            <button
              type="button"
              key={idx}
              className="group h-full w-5/6 rounded-r-full py-3 pl-4 transition-all duration-200 hover:bg-kanbanVeryLightGrey dark:hover:bg-white"
            >
              <li className="flex flex-row items-center justify-start gap-2">
                ...
                <span className="text-kanbanLightGrey transition-all duration-200 group-hover:text-kanbanPurpule">
                  {board.boardName}
                </span>
              </li>
            </button>
